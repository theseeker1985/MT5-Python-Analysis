...
def parse_html_report(file_path):
    ...
    # Extract trades with enhanced parsing
    trades = []
...
def generate_advanced_plots(df, settings, results):
    ...
    plt.suptitle(f"Advanced Strategy Analysis\n{settings.get('Expert', '')} on {settings.get('Symbol', '')}", y=1.02, fontsize=16)
    ...
    if 'Result' in df.columns:
        ...
        df['Streak'] = df['Result Binary'].groupby((df['Result Binary'] != df['Result Binary'].shift()).cumsum()).cumcount() + 1
